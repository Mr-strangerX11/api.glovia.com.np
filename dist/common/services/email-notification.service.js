...
let EmailNotificationService = EmailNotificationService_1 = class EmailNotificationService {
    ...
    buildOrderStatusChangedHtml(payload) {
    ...
    async sendVendorOrderEmailAsync(payload) {
        ...
        try {
            ...
            this.logger.error(`Failed to send vendor order email.\nRecipients: ${recipients}\nSubject: ${subject}\nPayload: ${JSON.stringify(payload, null, 2)}`, error);
            throw error;
        }
    }
    buildVendorOrderHtml(payload) {
        const itemsHtml = payload.items
            .map((item) => `
          <tr>
            <td style="padding: 12px 8px; border-bottom: 1px solid #f1f1f1;">${item.name}</td>
    ...
    async sendNewsletterSubscriptionEmail(customerEmail, customerName) {
        ...
        try {
            ...
            this.logger.error(`Failed to send newsletter subscription email.\nRecipients: ${recipients}\nSubject: ${subject}`, error);
            throw error;
        }
    }
    buildNewsletterSubscriptionHtml(email, name) {
    ...
    async sendVerificationEmail(customerEmail, customerName, userId) {
    ...
    buildVerificationEmailHtml(customerName, verificationLink) {
        return `
      <div style="background: #f4f6fb; padding: 0; margin: 0;">
        <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.04); font-family: Arial, sans-serif;">
          <!-- Header -->
    ...
    async sendVendorOrderNotificationEmail(vendorEmail, vendorName, orderData) {
        ...
        try {
            ...
            this.logger.error(`Failed to send vendor order notification email.\nVendor: ${vendorEmail}\nOrder: ${orderData.orderNumber}`, error);
            throw error;
        }
    }
    buildVendorOrderNotificationHtml(vendorName, orderData) {
