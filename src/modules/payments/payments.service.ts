...
export class PaymentsService {
  ...
  async verifyEsewaPayment(data: { oid: string; amt: string; refId: string }) {
    const order = await this.orderModel.findOne({ orderNumber: data.oid }).lean();
  ...
  async verifyKhaltiPayment(data: { token: string; amount: number; orderId: string }) {
  ...
  async verifyIMEPayment(data: { TransactionId: string; RefId: string; Msisdn: string }) {
    const order = await this.orderModel.findOne({ orderNumber: data.RefId }).lean();
    ...
    try {
      const response = await axios.post(`${this.configService.get('IME_GATEWAY_URL')}/Validate`, {
